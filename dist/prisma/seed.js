...
async function main() {
    ...
    const habits = [
        ...
        },
        {
            ...
            goalValue: 0,
            unitKey: 'sessions',
            direction: client_1.HabitDirection.NEGATIVE,
            iconName: 'strokeRoundedApple',
        },
        {
            title: 'Sleep 8 Hours',
    ...
    for (const h of habits) {
        const existing = await prisma.task.findFirst({
            where: { userId: systemUser.id, title: h.title, type: client_1.TaskType.HABIT, isPredefined: true },
        });
        if (existing)
            continue;
    ...
    for (const r of routines) {
        const existing = await prisma.task.findFirst({
            where: { userId: systemUser.id, title: r.title, type: client_1.TaskType.ROUTINE, isPredefined: true },
        });
        if (existing)
            continue;
        await prisma.task.create({
            data: {
                title: r.title,
                type: client_1.TaskType.ROUTINE,
                iconName: r.iconName,
                profile: { connect: { userId: systemUser.id } },
    ...
    for (const m of mindsets) {
        const existing = await prisma.task.findFirst({
            where: { userId: systemUser.id, title: m.title, type: client_1.TaskType.MINDSET, isPredefined: true },
        });
        if (existing)
            continue;
        await prisma.task.create({
            data: {
                title: m.title,
                type: client_1.TaskType.MINDSET,
                iconName: m.iconName,
                profile: { connect: { userId: systemUser.id } },
